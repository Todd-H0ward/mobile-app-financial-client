import { PET_STAGES, type PetStage } from '../../model';
...
/**
 * The growth formula, docs/pet.md.
 *
 ...
 *    `growPet` enforces it even when the facts say otherwise.
 ...
 */
const GROWTH_RULES: Record<PetStage, GrowthRule> = {
  baby: { periods: 0, goalsReached: 0, plansKept: 0 },
  teen: { periods: 2, goalsReached: 1, plansKept: 1 },
  adult: { periods: 4, goalsReached: 2, plansKept: 3 },
};
...
const isEarned = (stage: PetStage, facts: GrowthFacts) => {
...
/** How far along the tuple a stage sits. `baby` is 0. */
const rank = (stage: PetStage) => PET_STAGES.indexOf(stage);
 ...
 *
 * Reads the tuple from the far end, so a child who cleared the adult
 * conditions is an adult even if they somehow skipped a teen condition.
 */
export const stageFor = (facts: GrowthFacts): PetStage =>
  [...PET_STAGES].reverse().find((stage) => isEarned(stage, facts)) ?? 'baby';

/**
 * The stage a pet holds after settlement.
 *
 ...
 */
export const growPet = (current: PetStage, facts: GrowthFacts): PetStage => {
...
export const progressToNextStage = (
  ...
  const next = PET_STAGES[rank(current) + 1];
  if (!next) return null;
